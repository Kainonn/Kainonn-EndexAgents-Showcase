...
use App\Agents\Prospecting\TiqueAgent;
...
class RunTiqueOpportunityJob implements ShouldBeUnique, ShouldQueue
{
    use Queueable;
    ...
    public function handle(TiqueAgent $agent, LeadStateMachine $stateMachine): void
    {
        $lead = Lead::query()
        ...
        $lead->update([
            ...
            'processing_current_stage' => 'Minos',
            'processing_stage_number' => 4,
        ]);

        $stateMachine->transition($lead, LeadStatus::Enriched, 'agent', $result->agent);

        app(AnankeOrchestrator::class)->dispatchNextStage($lead->id, LeadStatus::Enriched);
    }

    public function failed(Throwable $exception): void
    {
        LeadPipelineFailure::handle($this->leadId, 'Tique', $exception);
    }
}
