...
use App\Agents\Compliance\HestiaAgent;
...
class RunHestiaComplianceJob implements ShouldBeUnique, ShouldQueue
{
    use Queueable;
    ...
    public function handle(HestiaAgent $agent, LeadStateMachine $stateMachine): void
    {
        ...
        $lead->update([
            ...
            'processing_current_stage' => 'Mnemosine',
            'processing_stage_number' => 10,
        ]);
        $stateMachine->transition($lead, LeadStatus::ComplianceChecked, 'agent', $result->agent);

        app(AnankeOrchestrator::class)->dispatchNextStage($lead->id, LeadStatus::ComplianceChecked);
    }

    public function failed(Throwable $exception): void
    {
        LeadPipelineFailure::handle($this->leadId, 'Hestia', $exception);
    }
}
